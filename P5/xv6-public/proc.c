...
found:
  ...
  // initializing values after adding nice system call
  p->nice = 0;
  p->original_nice = 0;
...
// Priority of process holding mutex lock elevated to same as calling process (priority inversion)
void priorityElevation(mutex *m){
  struct proc *p;

  // Lock ptable to iterate over processes and modify their priorities
  acquire(&ptable.lock);

  // Iterate over all processes in the process table.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if (m->pid == p->pid && p->nice > myproc()->nice) {
          p->nice = myproc()->nice; // elevate priority to match caller priority
      }
  }

  // Release the ptable lock after changes to priorities have been completed
  release(&ptable.lock);
}
